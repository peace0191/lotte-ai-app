...
import json
import os
from services.local_market import local_market_svc # Added for summary info
...
def render_dashboard():
    ...
    # ------------------------------------------------------------------------------
    # 🎓 PREMIUM REPORT: Integrated UI (Region + Education)
    # ------------------------------------------------------------------------------
    # 1. Title & Value Proposition
    st.markdown("<h3 style='text-align: center; margin-bottom: 10px;'>🎓 대치1동 프리미엄 리포트</h3>", unsafe_allow_html=True)
    st.markdown("""
    <div style="text-align: center; margin-bottom: 25px; color: #aaa;">
        대치동 중개실무 25년 차만 아는 <b>'진짜'</b> 입지 분석
    </div>
    <div style="background-color: rgba(46, 134, 222, 0.1); border-left: 5px solid #2e86de; padding: 15px; border-radius: 4px; margin-bottom: 25px;">
        <span style="font-size: 1.1em; font-weight: bold; color: #ffffff;">"아이의 통학 시간은 곧 수면 시간이고, 성적입니다."</span><br>
        <span style="font-size: 0.95em; color: #ddd;">대치1동은 유해시설이 전무한 '청정 교육 특구'이자, 자녀의 미래를 위한 최고의 베이스캠프입니다.</span>
    </div>
    """, unsafe_allow_html=True)

    # 2. Main Content Grid
    d_info = local_market_svc.get_daechi_summary()
    e_info = local_market_svc.get_education_summary()

    col_edu, col_feat = st.columns([1.3, 1])

    with col_edu:
        st.markdown("#### 🏫 학군 골드 라인 (Education Gold Line)")
        st.markdown("지도에 표시된 <span style='color:#ff8c00'>**주황색 화살표**</span>(대치초-래대팰-SK뷰)와 <span style='color:#ff1493'>**분홍색 화살표**</span>(대청-단대부-삼환)는 자녀에게 '시간'을 선물하는 핵심 동선입니다.", unsafe_allow_html=True)
        
        st.markdown("##### 📍 각 학교별 상세 특징")
        
        with st.container():
            # 1. Elementary
            st.markdown(f"**1. {e_info.get('elementary', ['서울대치초'])[0]}**")
            st.caption("특징: 높은 학업 성취도, 정확한 배정권역, '초품아' 프리미엄")
            
            # 2. Middle
            st.markdown(f"**2. {e_info.get('middle', ['대청중학교'])[0]}**")
            st.caption("특징: 특목고/자사고 진학률 최상위, 남녀공학 선호도 1위")
            
            # 3. High
            h_school = e_info.get('high', ['단대부고'])[0]
            st.markdown(f"**3. {h_school} / 숙명여고**")
            st.caption("특징: 서울대/의대 진학 실적 전국 TOP 클래스")

    with col_feat:
        st.markdown("#### 📌 대치1동 핵심 특성 (Key Traits)")
        
        # Region Characteristics Cards
        st.success(f"**📚 학원가**: {d_info.get('academy_street', '은마사거리~한티역')}")
        st.info(f"**🚇 교통**: {d_info.get('transport', '3호선/분당선')}")
        st.warning(f"**📈 동향**: {d_info.get('demand_trend', '이사철 전세 급증')}")
        
        st.markdown("#### 🛡️ 안심 생활권")
        st.write("대치1동 주민센터와 지구대가 인접하여 치안이 매우 우수하며, 유해시설(유흥가)이 전무합니다.")
