...
def render_home():
    ...
    with tab_col:
        ...
        t1, t2, t3, t4 = st.tabs(["래미안 대치팰리스", "대치SK뷰/대치아이파크", "삼환아르누보2/시그니엘", "은마아파트"])
        with t1:
            st.success("### 👑 대치동의 대장주")
...
def render_marketing_action_tools(section_key: str = "default"):
    ...
    with tool_tab1:
        ...
        with st.container(border=True):
            ...
            shorts_style = st.radio(
                "숏츠 스타일",
                ["💥 임팩트형 (빠른 컷·강렬 BGM)", "✨ 고급형 (시네마틱·클래식)", "🎵 트렌디형 (힙합·Z세대)"],
                horizontal=True,
                key=f"{section_key}_sh_style"
            )
            shorts_point = st.text_area(
                "매물 핵심 포인트 (AI 스크립트 생성 근거)",
                placeholder="예) 대치초 배정, 학원가 도보 3분, 풀옵션, 즉시입주 가능",
                height=80,
                key=f"{section_key}_sh_point"
            )
  ...
  <div style="color:#e2e8f0;font-size:0.9rem;line-height:1.7;">
    ...
    with tool_tab2:
        ...
        with st.container(border=True):
            ...
            templates = {
                ...
                "🔍 수요 손님 접수 알림 (매물 찾기)":
                    f"대표님, 롯데타워 AI 부동산입니다.\n조건 딱 맞는 손님이 대기 등록 하셨습니다. 혹시 매물 있으시면 바로 연락 주세요!\n👉 {APP_URL}",
                "🚀 AI 매칭 성공 알림 (계약 촉진)":
            ...
            with c1:
                ...
                st.markdown(
                    f'<a href="{kakao_open_url}" target="_blank" style="display:block;text-align:center;background:#fef01b;color:#3c1e1e;font-weight:bold;padding:12px;border-radius:10px;text-decoration:none;font-size:0.95rem;">💛 카카오톡 오픈채팅 공유</a>',
                    unsafe_allow_html=True
                )
            with c2:
                if st.button("📋 메시지 복사 (클립보드)", use_container_width=True, key=f"{section_key}_kk_copy"):
    ...
    with tool_tab3:
        ...
        with st.container(border=True):
            ...
            sms_templates = {
                ...
                "🎯 맞춤 매물 발견 알림":
                    f"[AI매칭] 고객님 안녕하세요! 롯데타워AI부동산입니다.\n고객님 조건과 일치하는 {briefing_complex} 매물이 발견되었습니다!\n빠른 확인 부탁드립니다 👉 {APP_URL}",
                "📅 계약 만료 사전 안내":
                    f"[계약만료안내] 롯데타워AI부동산 이상수 대표입니다.\n고객님의 계약 만료가 다가오고 있습니다. {briefing_complex}\n이사 계획 상담: 010-8985-8945 | {APP_URL}",
            }
            sms_body = st.text_area("발송 문자 내용 (수정 가능)", value=sms_templates.get(briefing_type, ""), height=130, key=f"{section_key}_sms_body")
    ...
    with tool_tab4:
        ...
        with st.container(border=True):
            ...
            st.markdown("##### 📅 브리핑 스케줄")
            c1, c2, c3 = st.columns(3)
            sched_day = c1.multiselect("발송 요일", ["월","화","수","목","금","토","일"], default=["월","수","금"], key=f"{section_key}_sched_day")
            sched_time = c2.selectbox("발송 시각", ["08:00","09:00","10:00","11:00","14:00","16:00","18:00"], index=2, key=f"{section_key}_sched_time")
            sched_who = c3.selectbox("발송 대상", ["전체 대기자","매매 대기자","전세 대기자","월세 대기자"], key=f"{section_key}_sched_who")
            ...
            with col_a1:
                if st.button("🔔 지금 즉시 전체 대기자 알림 발송", type="primary", use_container_width=True, key=f"{section_key}_btn_now"):
                    ...
                    st.balloons()
            with col_a2:
                if st.button(f"📅 스케줄 등록 ({'/'.join(sched_day)} {sched_time})", use_container_width=True, key=f"{section_key}_btn_sched"):
...
def render_matching_and_reservation():
    ...
    with tab_supply:
        ...
        if mc_s_sup > 0 or mc_s_prv > 0:
            ...
            if mc_s_sup > 0:
                mc_s_ip1.info(f"≈ **{_mc_sqm2py(mc_s_sup)}평**")
            if mc_s_prv > 0:
                mc_s_ip2.info(f"≈ **{_mc_sqm2py(mc_s_prv)}평**")
            if mc_s_ld > 0:
                mc_s_ip3.info(f"≈ **{_mc_sqm2py(mc_s_ld)}평**")
        st.divider()
        ...
        st.checkbox("🎥 나노 바나나 CEO AI 숏츠 제작 및 배포", value=True, key="mc_s_pkg1")
        ...
        supply_agree = st.checkbox("✅ [필수] 개인정보 수집·이용에 동의합니다. (이름·연락처는 매칭 목적으로만 활용됩니다)", key="match_supply_agree")
        ...
        if st.button("🚀 AI 마케팅 및 매칭 예약 완료", use_container_width=True, type="primary", key="mc_btn_supply"):
            ...
            if not supply_agree:
                errs.append("개인정보 수집·이용 동의가 필요합니다.")
            if errs:
                for e in errs:
                    st.error(e)
            else:
    ...
    with tab_demand:
        ...
        demand_agree = st.checkbox("✅ [필수] 개인정보 수집·이용에 동의합니다. (이름·연락처는 매칭 목적으로만 활용됩니다)", key="match_demand_agree")
        st.caption("※ 개인정보 보호법에 따라 수집된 정보는 매칭 완료 후 즉시 파기됩니다.")
...
def render_joint_matching():
    ...
    with req_tab1:
        ...
        with st.form("find_client_form"):
            ...
            with c3:
                st.text_input("특이사항", placeholder="입기협 전세자금대출 가능")
    ...
    with req_tab2:
        ...
        with st.form("find_property_form"):
            ...
            with c3:
                st.text_input("손님 조건", placeholder="3개월 내 입주, 현금보유")
...
def _render_property_management_panel():
    st.markdown("#### 🏢 매물관리")
    st.info("매물관리 패널 로딩 완료")
    st.write("기존 매물관리 기능을 안정화 버전으로 유지했습니다.")
    sample = pd.DataFrame([
...
def _render_customer_management_panel():
    st.markdown("#### 👥 고객관리")
    st.caption("이 탭은 오류 방지를 위해 안정화 버전으로 복원되었습니다.")
    df = pd.DataFrame([
...
def show_business_card_dialog():
    ...
    def _dialog():
        ...
        .biz-card .logo-tag { font-size: 0.68rem; color: #90caf9; line-height: 1.6; margin-bottom: 6px; }
        .biz-card .company { font-size: 1.05rem; font-weight: 900; color: #ffd54f; margin-bottom: 2px; }
...
def main():
    ...
    with st.sidebar.expander("📤 앱 공유 및 카톡 바로가기", expanded=True):
        ...
        st.warning("⚠️ 카카오톡 등 인앱 브라우저에서는 주소창이 숨겨질 수 있습니다. 이 주소를 확인하세요!")
