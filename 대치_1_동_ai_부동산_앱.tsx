...
const MENU = {
    ...
    recommend: { key: "recommend", label: "추천매물", icon: Sparkles },
...
// ✅ 이미지 폴백 처리를 위한 컴포넌트 추가
function AssetImage({ src, alt, fallback }: { src: string; alt: string; fallback: string }) {
    const [error, setError] = useState(false);
    return (
        <div className="rounded-2xl border overflow-hidden aspect-[4/5] bg-muted flex items-center justify-center">
            {error ? (
                <div className="p-3 text-xs text-muted-foreground text-center">{fallback}</div>
            ) : (
                <img
                    src={src}
                    alt={alt}
                ...
                <Card className="rounded-2xl shadow-sm md:col-span-5 overflow-hidden">
                    <div className="grid grid-cols-2 gap-2 p-3">
                        ...
                        <AssetImage
                            ...
                            alt="CEO"
                            fallback="CEO 사진 (assets에 넣으면 표시)"
                        />
                        <AssetImage
                            src={BRAND.assets.businessCard}
                            alt="Business Card"
                            fallback="명함 (assets에 넣으면 표시)"
                        />
                    </div>
                    <div className="px-4 pb-4">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground leading-relaxed" id="about">
                            안녕하세요. {BRAND.officeName} 대표 {BRAND.ceoName}입니다.<br />
                            대치1동은 ‘학군’이 곧 ‘주거 선택’이 되는 교육특구입니다.<br />
                            저희는 이 국지적 특성을 정확히 구조화하고, AI가 저평가 매물을 빠르게 추천·예약·계약 매칭까지 연결하며,<br />
                    ...
                    <CardContent className="space-y-3">
                        ...
                        <div className="rounded-2xl border p-3 text-sm">
                            ...
                            <div className="text-sm text-muted-foreground mt-1 leading-relaxed">
                                • {child} 자녀 · {gender} · {budget} 조건에서, 학군 라인 적합도가 높은 단지를 우선 추천합니다.<br />
                                • 추천매물의 ‘AI 신호’가 높은 매물부터 예약 슬롯을 자동 배정합니다.<br />
                ...
                <TabsContent value="demand" className="mt-4">
                    <Card className="rounded-2xl shadow-sm">
                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            {/* ✅ 제출 피드백 추가 */}
                            <Button className="rounded-xl md:col-span-2" onClick={() => setSubmitted(true)}>
                                <CalendarDays className="h-4 w-4 mr-2" />
                                {submitted ? "✅ 제출 완료 (데모)" : "예약 요청 제출"}
                            </Button>
...
export default function DaechiAIDemoApp() {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground">
            ...
            {active === MENU.recommend.key && <Recommend />}
