...
/* ─── 어두운 배경(dark-box) 내부 heading → 무조건 흰색으로 덮어쓰기 ─── */
.dark-box h1, .dark-box h2, .dark-box h3, .dark-box h4, .dark-box h5, .dark-box h6,
[style*="background:#0f172a"] h1, [style*="background:#0f172a"] h2,
[style*="background:#0f172a"] h3, [style*="background:#0f172a"] h4,
[style*="background:#1e293b"] h1, [style*="background:#1e293b"] h2,
[style*="background:#1e293b"] h3, [style*="background:#1e293b"] h4,
...
/* 어두운 배경 내 노란색 강조 heading 유지 */
.dark-box h4[style*="color:#fcd34d"],
.dark-box h4[style*="color: #fcd34d"],
[style*="color:#fcd34d"].dark-box h4 {
...
.strategy-card h4 { color: #0f172a !important; margin: 0 0 8px 0; }
.strategy-card p { color: #475569 !important; margin: 0; font-size: 0.9rem; }
...
@media (max-width: 768px) {
    ...
    .ext-link { font-size: 12px; padding: 10px 6px; }
...
.lnb-btn:active, .llnb-btn:active { transform: translateY(0); }
...
def render_daechi_map_block():
    ...
    column_layer = pdk.Layer("ColumnLayer", data=df, get_position=["lon", "lat"],
        get_elevation="height", elevation_scale=1, radius=35, get_fill_color="color",
        pickable=True, auto_highlight=True, extruded=True)
    halo_layer = pdk.Layer("ScatterplotLayer", data=df, get_position=["lon", "lat"],
        get_fill_color=[0, 0, 0, 50], get_line_color="color", stroked=True, filled=True,
        get_radius=70, radius_units="meters", line_width_min_pixels=2)
    label_layer = pdk.Layer("TextLayer", data=df, get_position=["lon", "lat"],
        get_text="display_name", get_size=15, get_color=[255, 255, 255],
        get_text_anchor="'middle'", get_alignment_baseline="'bottom'",
        get_pixel_offset=[0, -30], billboard=True,
        get_background_color=[0, 0, 0, 140], background_padding=[4, 2, 4, 2])

    tooltip = {"html": "<b>{name}</b><br/>{category}<br/>{note}",
               "style": {"backgroundColor": "rgba(20,20,20,0.9)", "color": "white"}}
    view_state = pdk.ViewState(latitude=df["lat"].mean(), longitude=df["lon"].mean(),
...
def render_realtime_search_panel(avg_prices):
    for key, default in [("ai_search_auto", True), ("ai_search_count", random.randint(1180, 1260)),
                          ("ai_last_search", time.time()), ("ai_flash_deals", random.randint(2, 5)),
    ...
    with btn_col1:
        if st.button("🔄 지금 탐색하기", use_container_width=True, key="btn_manual_search", type="primary"):
            ...
            new_log = f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 {random.choice(sources)} 수동 탐색 — {random.choice(topics)}"
...
def calculate_metrics():
    ...
    return {k: round(sum(buckets[k]) / len(buckets[k]), 1) if buckets[k] else defaults[k] for k in [20, 30, 40]}
...
def render_home():
    ...
    with tab_col:
        ...
        with t1:
            ...
            st.image("https://images.unsplash.com/photo-1600596542815-e328701102b9?auto=format&fit=crop&w=600&q=80", use_container_width=True)
        with t2:
            st.warning("### ⚖️ 실속과 환경의 조화")
            st.markdown("- 대치역/한티역 역세권, 백화점 슬세권\n- 쾌적한 주거 환경")
            st.image("https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=600&q=80", use_container_width=True)
        with t3:
            st.info("### 🏙️ 프리미엄 레지던스 & 오피스텔")
            st.markdown("- 시그니엘: 롯데월드타워 최고층 글로벌 레지던스\n- 삼환아르누보2: 학원가 도보 3분 오피스텔 가성비")
...
def render_listing():
    ...
    def property_card(title, price, desc, img_url, badge=None, key_suffix=None):
        ...
        with c2:
            if badge:
                st.markdown(f"<span style='background:#facc15; color:#1e293b; font-size:0.8em; padding:2px 6px; border-radius:4px; font-weight:800;'>{badge}</span>", unsafe_allow_html=True)
            st.markdown(f"#### {title}")
            st.markdown(f"**<span style='font-size:1.3rem; color:#0f172a;'>{price}</span>**", unsafe_allow_html=True)
            st.caption(desc)
    ...
    st.markdown("<hr style='border:none;border-top:2px dashed #334155;margin:20px 0;'>", unsafe_allow_html=True)
    st.markdown("#### 🏘️ 단지별 전체 추천매물 (부동산 등록 매물)")

    st.markdown('<div class="section-header">💎 시그니엘 레지던스 (롯데월드타워몰)</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: property_card("시그니엘 89평 프리미엄", "1억 / 1,700만", "89평 · 고층 · 남향 · 한강뷰 풀옵션", "https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=400&q=80", "월세", "sig89")
    with c2: property_card("시그니엘 88평 급매", "69억", "88평 · 고층 · 급매", "https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=400&q=80", key_suffix="sig88")
    with c3: property_card("시그니엘 95평 월세", "3억 / 1,800만", "95평 · 중층 · 입주협의", "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=400&q=80", key_suffix="sig95")

    st.markdown('<div class="section-header">🏫 래미안 대치팰리스 (대장주)</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: property_card("대치팰리스 45평 고층", "55억", "45평 · 고층 · 남향 · 대치중심", "https://images.unsplash.com/photo-1600596542815-e328701102b9?auto=format&fit=crop&w=400&q=80", "강력추천", "rdp45")
    with c2: property_card("대치팰리스 34평 고층뷰", "44억", "34평 · 고층 · 남향 · 한강조망", "https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?auto=format&fit=crop&w=400&q=80", key_suffix="rdp34")
    with c3: property_card("대치팰리스 33평 실용형", "42억", "33평 · 저층 · 남향 · 초급매", "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&w=400&q=80", key_suffix="rdp33")
    ...
    c1, c2, c3 = st.columns(3)
    with c1: property_card("대치아이파크 56평 펜트하우스", "59억", "56평 · 고층 · 남향 · 명문학군", "https://images.unsplash.com/photo-1512915922610-06c2b585408e?auto=format&fit=crop&w=400&q=80", "희소", "aip56")
    with c2: property_card("대치아이파크 33평 인기형", "40억", "33평 · 고층 · 남향 · 조망 우수", "https://images.unsplash.com/photo-1560185127-6ed189bf02f4?auto=format&fit=crop&w=400&q=80", key_suffix="aip33")
    with c3: property_card("대치아이파크 32평 실속형", "38억", "32평 · 저층 · 남향 · 가성비", "https://images.unsplash.com/photo-1484154218962-a1c002085d2f?auto=format&fit=crop&w=400&q=80", key_suffix="aip32")
    ...
    c1, c2, c3 = st.columns(3)
    with c1: property_card("삼환아르누보 17평 매매", "4.6억", "17평 · 서향 · 고층", "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?auto=format&fit=crop&w=400&q=80", "인기", "sam17")
...
def render_marketing_action_tools(section_key: str = "default"):
    ...
    tool_tab1, tool_tab2, tool_tab3, tool_tab4 = st.tabs([
        "🎬 숏츠·유튜버 광고", "💛 카카오톡 매칭 알리기",
        "📱 대기자 문자발송", "🔔 자동 알림 장치",
    ])

    with tool_tab1:
        st.markdown("#### 🎬 매물 숏츠 & 유튜버 광고 자동 제작")
        st.caption("AI가 매물 정보를 기반으로 유튜브/인스타 숏츠 스크립트를 자동 생성합니다.")
        with st.container(border=True):
            ...
            shorts_style = st.radio("숏츠 스타일", ["💥 임팩트형", "✨ 고급형", "🎵 트렌디형"], horizontal=True, key=f"{section_key}_sh_style")
            shorts_point = st.text_area("매물 핵심 포인트", placeholder="예) 대치초 배정, 학원가 도보 3분, 풀옵션, 즉시입주 가능", height=80, key=f"{section_key}_sh_point")
  ...
  <div style="color:#e2e8f0;font-size:0.9rem;line-height:1.7;">
    ...
    with tool_tab2:
        ...
        with st.container(border=True):
            ...
            templates = {
                "📢 공급 매물 접수 알림 (공동중개 요청)": f"안녕하세요! 롯데타워 AI 부동산 이상수 대표입니다.\n방금 전 우수 매물이 접수되었습니다. 공동중개 가능하신 분 연락주세요!\n👉 {APP_URL}",
                "🔍 수요 손님 접수 알림 (매물 찾기)": f"대표님, 롯데타워 AI 부동산입니다.\n조건 딱 맞는 손님이 대기 등록 하셨습니다. 매물 있으시면 바로 연락 주세요!\n👉 {APP_URL}",
                "🚀 AI 매칭 성공 알림 (계약 촉진)": f"[AI 매칭 성공 🎉] 롯데타워 AI 부동산입니다.\n귀하의 매물/수요 조건과 99% 일치하는 상대방이 발견되었습니다!\n지금 바로 확인하세요 👉 {APP_URL}",
                "✉️ 직접 작성": "",
            }
            default_msg = templates.get(kakao_msg_template, "")
            kakao_body = st.text_area("발송 메시지 (수정 가능)", value=default_msg, height=120, key=f"{section_key}_kk_body")
            c1, c2 = st.columns(2)
            with c1:
                kakao_open_url = f"https://open.kakao.com/o/share?url={APP_URL}"
                st.markdown(f'<a href="{kakao_open_url}" target="_blank" style="display:block;text-align:center;background:#fef01b;color:#3c1e1e;font-weight:bold;padding:12px;border-radius:10px;text-decoration:none;font-size:0.95rem;">💛 카카오톡 공유</a>', unsafe_allow_html=True)
            with c2:
    ...
    with tool_tab3:
        ...
        with st.container(border=True):
            ...
            briefing_complex = st.text_input("대상 단지/지역", placeholder="대치동 래미안대치팰리스 34평", key=f"{section_key}_sms_complex")
            briefing_point = st.text_input("핵심 브리핑 포인트", placeholder="학원가 3분, 8억 전세, 즉시입주", key=f"{section_key}_sms_point")
            sms_body = st.text_area("발송 문자 내용", value=f"[롯데타워AI부동산] {briefing_complex} 신규 매물 출시!\n{briefing_point}\n상세: {APP_URL}", height=100, key=f"{section_key}_sms_body")
            c1, c2, c3 = st.columns(3)
            ...
            with col_s1:
                if st.button("📨 1:1 문자 발송 (데모)", use_container_width=True, type="primary", key=f"{section_key}_btn_sms1"):
                    if not recv_phone:
                        st.warning("수신 번호를 입력해주세요.")
                    else:
                        with st.spinner("문자 발송 중..."):
                            time.sleep(0.8)
                        st.success(f"✅ {recv_name or recv_phone}님께 문자가 발송되었습니다! (데모)")
            with col_s2:
                if st.button(f"📢 대기자 {send_count}명 일괄발송 (데모)", use_container_width=True, key=f"{section_key}_btn_sms_bulk"):
                    with st.spinner(f"대기자 {send_count}명에게 발송 중..."):
                        time.sleep(1.0)
                    st.success(f"✅ {send_count}명의 대기자에게 문자 발송 완료! (데모)")
    ...
    with tool_tab4:
        ...
        with st.container(border=True):
            ...
            with col_a1:
                if st.button("🔔 지금 즉시 전체 대기자 알림 발송", type="primary", use_container_width=True, key=f"{section_key}_btn_now"):
                    with st.spinner("전체 대기자에게 알림 발송 중..."):
                        time.sleep(1.2)
                    st.success("✅ 4,218명에게 알림이 발송되었습니다! (데모)")
                    st.balloons()
            with col_a2:
                if st.button("📅 스케줄 등록", use_container_width=True, key=f"{section_key}_btn_sched"):
                    st.success("✅ 자동 브리핑 스케줄이 등록되었습니다!")
...
def render_matching_and_reservation():
    with st.expander("💬 AI 챗봇 상담 (열기/닫기)", expanded=True):
        ...
        with faq_tab1:
            ...
            if fq1.button("🚨 급매물 알림을 가장 먼저 받으려면?", use_container_width=True):
                selected_question = "급매물 알림 신청 방법 알려줘"
            if fq2.button("🏗️ 은마아파트 재건축 현재 단계는?", use_container_width=True):
                selected_question = "은마아파트 재건축 진행 현황 알려줘"
        with faq_tab2:
            fq1, fq2 = st.columns(2)
    ...
    _MC_FEATURES_S = ["💰 금액조절 가능","👀 바로 볼 수 있는","🏗️ 새로 지은","🙋 손님 대기중","🚇 역세권 위치","🔧 수리 깨끗한","🏦 전세대출 가능","🛋️ 풀옵션"]
    _MC_FEATURES_D = ["💰 금액조절 가능","🚀 즉시입주 가능","🚗 주차 필수","🏫 학군 중요","🚇 역세권 선호","🏗️ 신축 선호","🏦 대출 활용 예정","🏠 실거주 목적"]
    _MC_REGIONS_GU = ["강남구","서초구","송파구","강동구","마포구","용산구","성동구","광진구","강서구","양천구","영등포구","동작구","관악구","서대문구","은평구","노원구"]
    _MC_COMPLEX_OPTS = ["래미안대치팰리스","대치SK뷰","대치아이파크","은마아파트","삼환아르누보2 오피스텔","롯데월드타워몰 시그니엘레지던스","직접입력"]

    def _mc_sqm2py(v):
        return round(v / 3.3058, 2)
    ...
    with tab_supply:
        ...
        mc_s_complex = st.text_input("단지명 직접 입력", placeholder="단지명/건물명을 입력하세요", key="mc_s_complex") if mc_s_cplx_sel == "직접입력" else mc_s_cplx_sel
        ...
        for _i, _f in enumerate(_MC_FEATURES_S):
            if mc_s_fcs[_i % 4].checkbox(_f, key=f"mc_sf_{_i}"):
                mc_s_feats.append(_f)
        st.divider()

        st.markdown("##### 📅 일정 & 특이사항")
        mc_s_dc1, mc_s_dc2 = st.columns(2)
        mc_s_date = mc_s_dc1.date_input("이사예정일/인도가능일", value=datetime.date.today() + datetime.timedelta(days=30), key="mc_s_date")
        ...
        st.markdown("##### 🎁 AI 공급자 패키지")
        st.checkbox("🎥 AI 숏츠 제작 및 배포", value=True, key="mc_s_pkg1")
        st.checkbox("📊 주변 단지 대비 저평가 분석 리포트 생성", value=True, key="mc_s_pkg2")
        st.checkbox("👑 VIP 대기 수요자(4,200명) 우선 매칭 알림", value=True, key="mc_s_pkg3")
        st.divider()

        supply_agree = st.checkbox("✅ [필수] 개인정보 수집·이용에 동의합니다.", key="match_supply_agree")
        ...
        if st.button("🚀 AI 마케팅 및 매칭 예약 완료", use_container_width=True, type="primary", key="mc_btn_supply"):
            errs = []
            if not mc_s_name: errs.append("이름을 입력해주세요.")
            if not mc_s_phone: errs.append("연락처를 입력해주세요.")
            if not mc_s_complex: errs.append("단지명을 입력해주세요.")
            if not supply_agree: errs.append("개인정보 수집·이용 동의가 필요합니다.")
            if errs:
                for e in errs: st.error(e)
            else:
                with st.spinner("🔒 암호화 전송 중..."):
                    time.sleep(1.0)
                ...
                <div style="background:#1e3a8a; padding:20px; border-radius:10px;
                    ...
                    <div style="font-size:0.9em; color:#93c5fd;">AI 기반 매칭 예상 점수</div>
                    <div style="font-size:2.5em; font-weight:bold; color:#facc15;">94 / 100</div>
                    <div style="font-size:0.8em; margin-top:10px; color:#e2e8f0;">
                    🚨 <b>코멘트:</b> 현재 대치동 학군지 인근 수요가 급증하고 있어,<br>
                    등록하신 가격대는 '1주일 내 계약' 확률이 매우 높습니다!
                    </div>
                </div>
                """, unsafe_allow_html=True)
                st.balloons()
    ...
    with tab_demand:
        ...
        mc_d_complex = st.text_input("단지명 직접 입력", placeholder="단지명/건물명을 입력하세요", key="mc_d_complex") if mc_d_cplx_sel == "직접입력" else mc_d_cplx_sel
        st.divider()

        mc_d_fc1, mc_d_fc2, mc_d_fc3 = st.columns(3)
        mc_d_floor_pref = mc_d_fc1.selectbox("선호 층", ["무관","저층(1~5)","중층(6~15)","고층(16층+)"], key="mc_d_fpref")
        ...
        for _i, _f in enumerate(_MC_FEATURES_D):
            if mc_d_fcs[_i % 4].checkbox(_f, key=f"mc_df_{_i}"):
                mc_d_feats.append(_f)
        st.divider()

        mc_d_dc1, mc_d_dc2 = st.columns(2)
        mc_d_date = mc_d_dc1.date_input("입주 희망일", value=datetime.date.today() + datetime.timedelta(days=60), key="mc_d_date")
        ...
        demand_agree = st.checkbox("✅ [필수] 개인정보 수집·이용에 동의합니다.", key="match_demand_agree")
        st.caption("※ 개인정보 보호법에 따라 수집된 정보는 매칭 완료 후 즉시 파기됩니다.")
        if st.button("🔍 VIP 매칭 대기 등록하기", use_container_width=True, type="primary", key="mc_btn_demand"):
            errs = []
            if not mc_d_name: errs.append("이름을 입력해주세요.")
            if not mc_d_phone: errs.append("연락처를 입력해주세요.")
            if not mc_d_region: errs.append("희망 지역을 입력해주세요.")
            if not demand_agree: errs.append("개인정보 수집·이용 동의가 필요합니다.")
            if errs:
                for e in errs: st.error(e)
            else:
                with st.spinner("🔒 암호화 전송 중..."):
...
def render_joint_matching():
    ...
    with req_tab1:
        ...
        with st.form("find_client_form"):
            ...
            with c3: st.text_input("특이사항", placeholder="전세자금대출 가능")
    ...
    data = {
        ...
        "제목 (AI 요약)": ["래대팰 45평 판상형 매물 보유 (진공인)", "SK뷰 34평 전세 손님 대기 (대박부동산)", "은마 31평 급매 찾으시는 분", "학원가 50평 임대 맞춤 가능", "대치아이파크 매수 손님 의뢰"],
...
def render_admin_system():
    ...
    with adm_tab2:
        ...
        c1, c2 = st.columns(2)
        with c1: st.metric("전체 고객", "128명")
        with c2: st.metric("매칭 진행", "23건")
    ...
    with adm_tab3:
        ...
        with st.form("admin_sales_pack_form"):
            ...
            _SP_COMPLEX = ["래미안대치팰리스","대치SK뷰","대치아이파크","은마아파트","삼환아르누보2 오피스텔","롯데월드타워몰 시그니엘레지던스","직접입력"]
            sp_lc1, sp_lc2 = st.columns(2)
            sp_cplx_sel = sp_lc1.selectbox("단지명/건물명", _SP_COMPLEX, index=1, key="sp_cplx_sel")
            sp_dongho = sp_lc2.text_input("동/호수 (비공개)", placeholder="101동 1501호", key="sp_dongho")
            sp_complex = st.text_input("단지명 직접 입력", placeholder="단지명/건물명을 입력하세요", key="sp_complex") if sp_cplx_sel == "직접입력" else sp_cplx_sel
            ...
            if submit:
                ...
                <div class="dark-box" style="background:#0f172a; padding:20px; border-radius:10px;
                    ...
                    <p style="color:#e2e8f0; margin:0; line-height:1.8;">
                        ...
                        {sp_complex} {sp_pyg} {sp_floor}층 물건이 방금 접수되었습니다.<br>
                        빠른 검토 부탁드립니다. ☎ {sp_phone}
                    </p>
                </div>
                """, unsafe_allow_html=True)
                st.balloons()
    ...
    with adm_tab4:
        ...
        st.info("시스템 관리 패널입니다. 하단 바로가기 버튼으로 주요 기능에 접근하세요.")
        st.markdown("---")
        st.markdown("##### 🔗 주요 기능 바로가기")
...
def render_login_page():
    ...
    <div style="background:linear-gradient(135deg,#061537 0%,#081a45 100%);
        ...
        <div style="display:flex; align-items:flex-start; margin-bottom:18px; flex-wrap:wrap; gap:14px;">
            <div style="background:#fbbf24; width:72px; height:72px; border-radius:50%;
                        ...
                        font-size:34px; flex-shrink:0; box-shadow:0 6px 18px rgba(251,191,36,0.35);">👨‍💼</div>
        ...
        <div style="font-size:1.45rem; font-weight:900; line-height:1.5;
                    letter-spacing:-0.03em; color:#fde68a; margin-bottom:10px;">
    ...
    # ─── AI 핵심 3대 전략 (중복 없이 1회만 렌더링) ───
    st.markdown('<div id="ai-strategy-section"></div>', unsafe_allow_html=True)
    st.markdown("#### 🔷 AI 부동산 핵심 3대 전략")

    strategy_items = [
        ("#3b82f6", "🎓 교육특구 1번지 분석",
         "래대팰·SK뷰(대치초/단대부고) vs 아이파크(대도초/숙명여중고)<br>학군 정밀 분석 및 배정 원칙 데이터화"),
        ("#8b5cf6", "🧬 AI저평가 매물 매수·매도·임대차 예약 AI자동매칭",
         "빅데이터로 저평가 매물을 발굴하고, 매수·매도·임대차 예약 고객에게 <b>1초 만에 AI자동 매칭</b>하여 거래 성사율 극대화"),
        ("#ef4444", "📢 AI 자동 홍보 시스템",
         "매물 접수 즉시 영업 문구 자동 생성 및 타겟 고객 발송"),
    ]
...
<div style="background:linear-gradient(135deg,#FFF01E,#F9E000); border-radius:16px;
  ...
  <a href="https://open.kakao.com/o/share?url={APP_URL}" target="_blank"
     style="display:block; text-align:center; background:#3c1e1e; color:#FFF01E;
            font-weight:900; padding:12px; border-radius:10px; text-decoration:none;
            font-size:1rem;">
    💛 카카오톡으로 공유하기
  </a>
</div>
""", unsafe_allow_html=True)
    st.caption("💡 모바일/PC에서 앱 링크 공유용 안내 카드입니다.")
    ...
    def _dialog():
        ...
        <div style="background:linear-gradient(135deg,#003087,#1565c0); border-radius:14px;
            ...
            <div style="color:#fff; font-size:0.82rem; margin:2px 0;">
                E-mail : 5788285@naver.com &nbsp;|&nbsp; tel : 578-8285
            </div>
